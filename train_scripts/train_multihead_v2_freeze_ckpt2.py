...
if __name__ == "__main__":
    ...
    training_data = custom_dataset.CustomDataset("imat_data/train_annos_6-26_final.json", "Train", transforms=transforms)
    ...
    front_half_backbone_params = [p for n, p in model.named_parameters()
                      if p.requires_grad and "classifier" not in n and "features.6" not in n and "features.7" not in n and "features.8" not in n]    
            
    backhalf_backbone_params = [p for n, p in model.named_parameters() 
                      if p.requires_grad and "classifier" not in n and "features.6" in n or "features.7" in n or "features.8" in n]
    classifier_params = [p for n, p in model.named_parameters() 
                        if p.requires_grad and "classifier" in n]
    
    param_groups = [
        {'params': front_half_backbone_params, 'lr': lr * 0.1},  # Front half of the backbone with lower learning rate
        {'params': backhalf_backbone_params, 'lr': lr * 0.1},  # Backbone with lower learning rate
